...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/12/14 15:59:25 by aducas            #+#    #+#             */
/*   Updated: 2020/12/14 16:00:43 by aducas           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_ceiling_and_floor(t_cub3d cub3d, int x)
{
	...
	if (cub3d.config.rgb_ceiling != 0)
	{
		while (y < cub3d.win.y / 2)
			mlx_pixel_put(cub3d.win.mlx_ptr, cub3d.win.win_ptr, x,
				y++, cub3d.config.rgb_ceiling);
	}
	if (cub3d.config.rgb_floor != 0)
	{
		while (y != cub3d.win.y)
			mlx_pixel_put(cub3d.win.mlx_ptr, cub3d.win.win_ptr, x,
				y++, cub3d.config.rgb_floor);
	}
}
