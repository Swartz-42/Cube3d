...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/12/14 16:29:49 by aducas            #+#    #+#             */
/*   Updated: 2020/12/14 16:43:17 by aducas           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
static float	rayon_h2(t_cub3d cub3d, float ay, float ax, float ya, float xa)
{
	...
	{
		(ax < 0) ? ax = 0 : 0;
		((ax / BLOCK_SIZE) > x_max(cub3d, ay)) ?
			ax = x_max(cub3d, ay) * BLOCK_SIZE : 0;
		if (cub3d.config.map_ok[(int)ay / BLOCK_SIZE]
			[(int)ax / BLOCK_SIZE] == '1')
			return (sqrt(pow(cub3d.player.fposx - ax, 2)
			+ pow(cub3d.player.fposy - ay, 2)));
		ay += ya;
		ax += xa;
	}
}
...
static float	rayon_v2(t_cub3d cub3d, float ay, float ax, float ya, float xa)
{
	...
	{
		(ay < 0) ? ay = 0 : 0;
		((ay / BLOCK_SIZE) > cub3d.config.y_max) ? ay = cub3d.config.y_max
														* BLOCK_SIZE : 0;
		if (cub3d.config.map_ok[(int)ay / BLOCK_SIZE]
				[(int)ax / BLOCK_SIZE] == '1')
			return (sqrt(pow(cub3d.player.fposx - ax, 2)
